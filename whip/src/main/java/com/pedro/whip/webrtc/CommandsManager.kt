    ...
    private fun createBody(
        ...
        if (!videoDisabled) {
            val media = when (videoCodec) {
                VideoCodec.H264 -> SdpBody.createH264Body(rtpTracks.trackVideo, spsString, ppsString, true)
                VideoCodec.H265 -> SdpBody.createH265Body(rtpTracks.trackVideo, spsString, ppsString, vpsString, true)
                VideoCodec.AV1 -> SdpBody.createAV1Body(rtpTracks.trackVideo, true)
            }
            videoBody = media +
                "a=rtcp-mux\r\n" +
        ...
        return "v=0\r\n" +
                ...
                "a=msid-semantic: WMS\r\n" +
                // GPX patch: offer setup:actpass and pick the DTLS role from the answer's a=setup. Millicast
                // answers passive (it is the DTLS server), so we drive the handshake as DTLS client. See
                // WhipClient (DtlsClient vs DtlsConnection).
                "a=setup:actpass\r\n" +
                "a=ice-ufrag:$uFrag\r\n" +
                "a=ice-pwd:$uPass\r\n" +
